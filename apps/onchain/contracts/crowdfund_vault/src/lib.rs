...
impl CrowdfundVaultContract {
    ...
    pub fn add_subscriber(env: Env, admin: Address, subscriber: Address) -> Result<(), CrowdfundError> {
        ...
        let mut subscribers: Vec<Address> = env.storage().instance().get(&DataKey::Subscribers).unwrap_or(vec![&env]);
        if !subscribers.contains(&subscriber) {
            subscribers.push_back(subscriber);
            env.storage().instance().set(&DataKey::Subscribers, &subscribers);
        }
        Ok(())
    }
    ...
    fn notify_subscribers(env: &Env, event_type: Symbol, data: soroban_sdk::Bytes) {
        let subscribers: Vec<Address> = env.storage().instance().get(&DataKey::Subscribers).unwrap_or(vec![env]);
